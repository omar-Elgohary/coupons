...
<div class="h-screen flex bg-gray-200">
    ...
    <div class=" absolute z-50 right-0 bg-gray-800 text-white w-56 min-h-screen overflow-y-auto transition-transform transform translate-x-full ease-in-out duration-300" id="sidebar">
      ...
      <section class="flex-1 flex flex-col">
        <div class="bg-white shadow">
          <div class="container mx-auto">
            ...
            <button class="  bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-3 edit-btn rounded-[8px]"><a
                href="{{ route('addCoupon') }}">اضافة قسيمة</a>
            </button>
        ...
        <div class="container mx-auto">
          <h1 class="text-2xl mb-3 mt-3 font-bold">قائمة القسائم</h1>
        </div>
          ...
          <div class="overflow-x-auto sm:mx-0.5 lg:mx-0.5">
                ...
                <table class="min-w-full">
                  ...
                  <thead class="bg-white border-b">
                    <tr class="text-center">
                      ...
                      <th class="text-base font-semibold text-gray-900 px-6 py-4 text-righ">القسيمة
                      </th>
                      <th class="text-base font-semibold text-gray-900 px-6 py-4 text-righ">الحالة
                      </th>
            ...
            @forelse($coupons as $key => $item)
                ...
                <tr class="text-center bg-gray-100 border-b r">
                    ...
                    <td><img src="{{ asset('uploads/images/'.$item->company->image)}}" alt="" width="50" height="50" class="m-auto"></td>
                    <td class="text-base font-medium text-gray-900 px-6 py-4 text-center whitespace-nowrap">{{$item->company->name_ar}} - {{$item->company->name_en}}</td>
                    <td class="text-base font-medium text-gray-900 px-6 py-4 text-center whitespace-nowrap">{{$item->country->name}} - {{$item->country->nameAr}}</td>
                    <td class="text-base font-medium text-gray-900 px-6 py-4 text-center whitespace-nowrap">{{$item->title}}
                    </td>
                @if($item->status === 1)
                    <td class="text-success text-base font-medium text-gray-900 px-6 py-4 text-center whitespace-nowrap">
                        {{'مفعل'}}
                    </td>
                @else
                    <td class="text-danger text-base font-medium text-gray-900 px-6 py-4 text-center whitespace-nowrap">
                        {{'غير مفعل'}}
                    </td>
                @endif
